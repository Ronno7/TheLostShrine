...
The production kit follows the user-approved sample: broad flat shapes, restrained texture and **14 exact Hearth & Meadow colors**. It contains **31 objects**, **275 native 16×16 tile pieces**, **28 static prefabs**, and **three practice-art prefabs**, all at **16 pixels per unit**.

| Group | Included |
| --- | --- |
| Village | Timber workshop, thatched longhouse, woodshed, roofed stone well |
...
The replacement structures are a **6x5 workshop (96x80 pixels)** and an **8x6 longhouse (128x96 pixels)**, both at unit transform scale. They retain the existing Cottage_Clay and Cottage_Thatch asset names for stable references. The oak, boulder and horizontal fence preserve the approved sample pixels exactly. The stump has an empty hatchet notch; the pickup remains separate. Targets and dummy contain art only. Lighting, fog, shadows, camera visibility effects and the URP migration remain deferred.
...
- Buildings block their lower footprint; roof portions are overhead. Cottages block an 8×4 footprint, with their upper four rows overhead. Their centered doors sit at local x=4; use two path columns at x=3 and x=4. Extend the last path cells beneath the threshold to remove the grass gap. Buildings are closed exterior art; interiors/door interactions are a later task.
