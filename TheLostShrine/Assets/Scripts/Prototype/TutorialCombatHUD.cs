...
namespace TheLostShrine.Prototype
{
    public sealed class TutorialCombatHUD : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            GUI.Box(new Rect(12f, 12f, width, 144f), GUIContent.none);
            ...
            GUI.Label(new Rect(24f, 91f, width - 24f, 55f),
                "Mouse: aim  |  LMB: light combo\nHold RMB, release: spin  |  E: throw / recall\nFull gold charge breaks shields and cracked blocks.", text);

            if (weapon != null && weapon.State == HatchetState.Charging)
            {
                GUI.Box(new Rect(12f, 164f, width, 30f), GUIContent.none);
                Color previous = GUI.color;
                GUI.color = weapon.Charge01 >= 1f ? new Color(1f, 0.8f, 0.2f) : new Color(0.5f, 0.8f, 1f);
                GUI.DrawTexture(new Rect(18f, 170f, (width - 12f) * weapon.Charge01, 18f), Texture2D.whiteTexture);
                GUI.color = previous;
            }
        }
    }
